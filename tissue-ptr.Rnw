...
<<fig.show='hide',label=figure-5, dependson("initialize"),echo=FALSE,warning=FALSE>>=
...
Reliability of \Sexpr{focus.data} is: \Sexpr{p1.reliability}\\
Reliability of \Sexpr{data2} is: \Sexpr{p2.reliability}
...
\section*{Figures S1}
<<fig.show='hide',label=figure-s1, dependson("initialize"),echo=FALSE,warning=FALSE>>=
...
<<results='hide',fig.show='hide', label=figure-slope,dependson=c("initialize"),echo=FALSE,warning=FALSE>>=
slps <- c()
...
protein.vec <- as.numeric(t(protein-apply(protein,1,function(x) median(x,na.rm=TRUE))))
mrna.vec <- as.numeric(t(mrna-apply(mrna,1,function(x) median(x,na.rm=TRUE))))
plot(mrna.vec,protein.vec,pch=19,cex=0.1)
abline(a=0,b=1)
...
\section*{Figure 3}
<<results='hide',fig.show='hide',label=figure-3,dependson=c("initialize"),echo=FALSE,warning=FALSE>>=

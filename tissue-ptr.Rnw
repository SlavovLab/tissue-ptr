...
## cors.tmp <- cat.tmp <- c()
## for(grp in names(nms)) {
##     prots <- intersect(n2e(group2proteins[[grp]]),names(out$z.score))
##     cors.tmp <- c(cors.tmp,out$within.cors[prots])
##     cat.tmp <- c(cat.tmp,rep(grp,length(prots)))
## }
## cors.tmp <- c(cors.tmp,out$within.cors)
## cat.tmp <- c(cat.tmp,rep("All",length(out$within.cors)))
## df <- data.frame(list(cors=cors.tmp,cat=factor(cat.tmp,levels=unique(cat.tmp),ordered=TRUE)))

## ggplot(atable, aes(x = Name, y = Median.Correlation))+geom_hline(yintercept=0.28,colour="red",lwd=1.5) + geom_boxplot(aes(x=Name,y=Median.Correlation),outlier.size=0)
